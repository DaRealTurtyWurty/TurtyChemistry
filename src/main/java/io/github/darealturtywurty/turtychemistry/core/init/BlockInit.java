...
public final class BlockInit extends AbstractInit {
    ...
    public static final RegistryObject<ShaleFracturerBlock> SHALE_FRACTURER = BLOCKS.register("shale_fracturer",
            () -> new ShaleFracturerBlock<>(build(of(Material.METAL)).blockItem(true)));
    
    public static final RegistryObject<ShaleFracturerBlock.DrillBlock> SHALE_DRILL = BLOCKS.register("shale_drill",
            () -> new ShaleFracturerBlock.DrillBlock<>(build(of(Material.METAL).strength(-1f)).blockItem(false)));
    ...
    public static class BlockItems {
        ...
        public static void registerBlockItems(RegistryEvent.Register<Item> event) {
            BLOCKS.getEntries().stream().filter(RegistryObject::isPresent).map(RegistryObject::get)
                    .filter(BLOCK_ITEM_WHITELIST::containsKey).forEachOrdered(block -> {
                        final var blockItem = new BlockItem(block, new Item.Properties().tab(TurtyChemistry.TAB))
